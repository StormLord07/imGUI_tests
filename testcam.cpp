...
void PrintGUID(const GUID& guid) {
    std::wcout << std::hex << std::uppercase << std::setw(8) << guid.Data1
               << '-' << std::setw(4) << guid.Data2 << '-' << std::setw(4)
...
void PrintMediaType(IMFMediaType* pType) {
    ...
    std::wcout << L"Frame Rate: " << numerator << L"/" << denominator
               << std::endl;
}
...
int main() {
    ...
    CHK(pAttributes->SetGUID(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE,
    ...
    if (count == 0) {
        ...
        for (UINT32 i = 0; i < count; ++i) {
            ...
            std::wcout << L"Device " << i + 1 << L": " << szFriendlyName
                       << std::endl;
            ...
            CHK(MFCreateSourceReaderFromMediaSource(pSource, pAttributes,
                                                    &pReader));
